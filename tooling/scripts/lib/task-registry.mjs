...
export function loadTaskRegistry(root) {
  ...
  for (const project of parsed.projects) {
    assert(
      project && isJsPlainObject(project),
      "project entries must be objects",
    );
    assert(
      isJsString(project.id) && project.id.length > 0,
      "project.id is required",
    );
    assert(!ids.has(project.id), `duplicate project id: ${project.id}`);
    ids.add(project.id);
    assert(
      isJsString(project.path) && project.path.length > 0,
      `${project.id} path is required`,
    );
    assert(
      !project.path.startsWith("/") && !project.path.includes(".."),
      `${project.id} path must be repo-relative`,
    );
    ...
    assert(
      isJsBoolean(project.workspace),
      `${project.id} workspace must be boolean`,
    );
    assert(
      isJsBoolean(project.submodule),
      `${project.id} submodule must be boolean`,
    );
    assert(
      isJsBoolean(project.required),
      `${project.id} required must be boolean`,
    );
    assert(
      Array.isArray(project.capabilities),
      `${project.id} capabilities must be an array`,
    );
    for (const capability of project.capabilities) {
      assert(
        CAPABILITIES.has(capability),
        `${project.id} has unknown capability ${capability}`,
      );
    }
    assert(
      isJsString(project.owner) && project.owner.length > 0,
      `${project.id} owner is required`,
    );
