...
| `Idempotency-Key` | Sur les écritures qui bougent de l’argent | Obligatoire sur `POST /payment-requests`, `POST /refunds`, `POST /payouts`, `POST /settlements/instant` et `POST /charge/*`. Recommandé sur `POST /checkout-sessions` (optionnel pour que les apps existantes puissent créer une page hébergée sans cet en-tête). Voir [Clés d’idempotence](/build/reliability/idempotency-keys). |
...
Les réponses incluent `X-Request-Id` (et `request_id` dans les corps d’erreur). Vous n’envoyez pas d’en-tête de tracing.
