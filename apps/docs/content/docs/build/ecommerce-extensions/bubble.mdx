...
<Callout type="info" title="Related products">
  ...
  - **Embed widget**: [Embed widget](/build/accept/embed-widget), alternative when you want a button on a Bubble page
</Callout>
...
**Embed mode**: set the **lomi. Pay button** (or **Open checkout embed**) to `embed` to keep shoppers in your app; listen for `oncomplete` or the `lomi:checkout-complete` window event. This reuses the [embed widget](/build/accept/embed-widget).

**Webhook headers**: `X-Lomi-Event`, `X-Lomi-Signature` (HMAC-SHA256 hex of the raw body). Details: [Handling webhooks](/build/reliability/handling-webhooks) and [Webhooks](/build/reliability).
