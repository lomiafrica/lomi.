...
This page is integration advice: how to parse API errors in your app. The code list lives on [Errors](/api/errors).
...
Refer to the [Error reference](/api/errors) for a detailed list of status codes and common error responses.
...
import { LomiSDK, LomiError } from '@lomi./sdk'; // Assuming SDK exports error type
...
These occur when the `LOMI_SECRET_KEY` is missing, invalid, or doesn't have the necessary permissions.
...
try {
  ...
  if (error instanceof LomiApiError && error.statusCode === 401) {
    ...
    if (!process.env.LOMI_SECRET_KEY) {
      console.error(
        'Suggestion: LOMI_SECRET_KEY environment variable might be missing.',
      );
    } else {
      console.error(
        'Suggestion: Verify the LOMI_SECRET_KEY is correct and active.',
      );
    }
  } else {
    console.error('An unexpected error occurred:', error);
