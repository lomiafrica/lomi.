...
| `explanation` | Concepts et compromis | [Choisir l'intégration](/build/choose-integration), [Cycle de vie des paiements](/build/reliability/payment-lifecycle) |
...
La fin de page utilise seulement le pied précédent / suivant intégré. N’ajoutez pas de grille de cartes « étapes suivantes » ni de liste markdown `## Étapes suivantes`.
...
Quand vous changez la sémantique des statuts de paiement dans `apps/api`, mettez à jour [Cycle de vie des paiements](/build/reliability/payment-lifecycle) et lancez `pnpm docs:drift` dans `apps/docs`.
