...
description: 'Following these best practices helps maintain code quality, consistency, and security across lomi. project.'
---
...
Refer to the [API Environment documentation](/start/api-keys) for managing API keys.
...
Consider practices like [Webhook Signature Verification](/build/reliability/handling-webhooks) to ensure data integrity.
...
See the full [Branching Strategy](/resources/contributing/branching-strategy) for details.
...
Use comments to explain _why_ something is done, not _what_ it does (the code should explain the what). Use TSDoc for functions and complex logic.
...
Package versions are managed within their respective `package.json` files and updated according to our [Versioning](/resources/contributing/versioning) guide.
...
<DocsNextSteps>
  ...
  <DocsNextStep href="/start/support" hint="Ask the team">
    Getting support
  </DocsNextStep>
</DocsNextSteps>
