...
export function buildServerInstructions(
  ...
  const lines = [
    ...
    '- To contact lomi. (complaint, billing, integration, abuse, security): lomi_support action=file. Guest needs email + message. A merchant key opens a Settings → Support ticket (list/get/close). action=status returns https://status.lomi.africa and GET /ready. Merchant-only: action=export (GDPR bundle) and action=delete_account (confirmation_token then soft delete).',
    '- Switch organization with lomi_organization action=create or action=use; both adopt the returned secret on this session. Create extra keys with lomi_api_keys (does not switch). Team and checkout/storefront settings are lomi_team and lomi_settings.',
    '- Logs require type (api_request, api_error, webhook_delivery, activity). Webhook retries stay on lomi_webhooks. Exports include statement_pdf, journal_csv, customers_pdf, logs_csv, and webhook_deliveries_csv.',
