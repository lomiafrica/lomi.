...
async fn dispatch(command: cli::app::Commands, common: &cli::CommonOptions) -> anyhow::Result<()> {
    ...
    match command {
        ...
        Commands::Invoices(args) => commands::invoices::run(common, args).await,
        Commands::Customers(args) => commands::customers::run(common, args).await,
        Commands::Balance(args) => commands::balance::run(common, args).await,
        Commands::ApiKeys(args) => commands::api_keys::run(common, args).await,
        Commands::Exports(args) => commands::exports::run(common, args).await,
        Commands::Logs(args) => commands::logs::run(common, args).await,
        Commands::Open(args) => commands::open_cmd::run(common, args).await,
        Commands::Receipts(args) => commands::receipts::run(common, args).await,
